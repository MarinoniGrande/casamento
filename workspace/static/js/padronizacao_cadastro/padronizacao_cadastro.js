...
/**
 *  Função para validar os campos que possuírem o atributo data-required='true' dentro do modal de cadastro
 * @param e todos os dados referente a tabela
 * @returns quantidade de erros
 */
function validarCamposDataRequired(modal){
    let array_elementos_requireds = modal.find("*[data-required='true']");
    for(let erro of array_elementos_requireds){
        if(erro.tagName === 'INPUT' || erro.tagName === 'TEXTAREA'){
            if($(erro).attr('type') === 'checkbox'){
                if ($(erro).is(":checked") === false) {
                    $(erro).attr('data-errorequired', 'true');
                    $(erro).parent().find("label[for='" + erro.id + "']").addClass('border-danger');
                }
            }
            else if($(erro).attr('type') === 'radio'){
                if(modal.find(`*[name='${$(erro).attr('name')}']`).is(":checked") === false){
                    modal.find(`*[name='${$(erro).attr('name')}']`)
                        .attr('data-errorequired', 'true')
                        .parent().find("label").addClass('border-danger');
                }
            }
            else{
                if($(erro).val() === "") {
                    $(erro).attr('data-errorequired', 'true');
                    $(erro).addClass('border-danger');
                }
            }
        }
        else if(erro.tagName === 'SELECT'){
            if($(erro).val() === "" || $(erro).val() === null || $(erro).val() === undefined ||  $(erro).val() === "-1" )
            {
                $(erro).attr('data-errorequired', 'true');
                $(erro).parent().find('.select2-selection').addClass('border-danger');
            }
        }
    }
    return modal.find('*[data-errorequired="true"]').length;
}
 ...
 */
function limparInputsCadastro(modal) {
    modal.find('*').removeClass('border-danger').removeAttr('data-errorequired');
    ...
    modal.find('select').val("").trigger('change.select2');
 ...
 * @param e todos os dados referente a tabela
 */
function alterandoStatusDeSalvouConteudo(e, modal) {
    const inputs = modal.find("select, input, textarea");
    if (inputs.length > 0)
        for (let input of inputs) {
            //remove a classe border-danger dos componentes
            $(input).removeAttr('data-errorequired');
            ...
            $(input).on('change', function trocou() {
                ...
                //remove a classe border-danger dos componentes
                $(input).removeAttr('data-errorequired');
                if($(input).attr('data-required') === "true" || $(input).attr('data-required') === true){
                    if(input.tagName === 'INPUT' || input.tagName === 'TEXTAREA'){
                        if($(input).attr('type') === 'checkbox'){
                            if($(input).is(':checked')){
                                $(input).parent().find("label[for='"+input.id+"']").removeClass('border-danger');
                            }
                            else {
                                $(input).parent().find("label[for='"+input.id+"']").addClass('border-danger');
                            }
                        }
                        else if($(input).attr('type') === 'radio') {
                            if($(input).parent().find("label[for='"+input.id+"']").hasClass('border-danger')) {
                                modal.find(`*[name='${$(input).attr('name')}']`)
                                    .removeAttr('data-errorequired')
                                    .parent().find("label").removeClass('border-danger');
                            }
                        }
                        else {
                        ...
                        $(input).parent().find('.select2-selection').removeClass('border-danger');
                    }
                }

                _itsSave = false;
                modal.find('.seletorBtnSalvar').removeClass('isDisabled');
                if (modal.find("*[name='" + e.component.cadastro._key + "']").val() !== undefined &&
                    modal.find("*[name='" + e.component.cadastro._key + "']").val() !== null &&
                    modal.find("*[name='" + e.component.cadastro._key + "']").val() !== "") {
                    modal.find('.btnExcluir').removeClass('isDisabled');
                }
                else {
                    modal.find('.btnExcluir').addClass('isDisabled');
                }
                // modal de cadastro fechado
